...
> ***Reminder***: the commands below should be run from [Kata-BabyStepsGolf/csharp]() directory
...
> ***Reminder***: the command below should be run from [Kata-BabyStepsGolf/csharp]() directory
...
`Kata-BabyStepsGolf` / `csharp` / `Kata-BabyStepsGolf.sln`

After loading the solution:

1. Make sure the 'BabyStepsGolf.Test' project is set as the Start-up Project.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `csharp`
