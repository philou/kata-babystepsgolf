# Getting Started with BabyStepsGolf kata in PHP
...
> ***Reminder***: the commands below should be run from [Kata-BabyStepsGolf/php](.) directory
...
`File` > `Open` > `Kata-BabyStepsGolf` > `php`
...
> ***Reminder***: the command below should be run from [Kata-BabyStepsGolf/php](.) directory
...
Choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `php` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `php`
