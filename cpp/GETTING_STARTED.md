# Getting Started with BabyStepsGolf kata in C++
...
- Create a build directory: ***Kata-BabyStepsGolf/cpp/build***. All build-related files are generated under this directory.
...
- Generate the solution file ***Kata-BabyStepsGolf.sln*** for **Visual Studio 2022** on Windows,
  or the project file ***Kata-BabyStepsGolf.xcodeproj*** for **Xcode** on macOS.
...
> ***Reminder***: the commands below should be run from [Kata-BabyStepsGolf/cpp]() directory
...
> ***Reminder***: the command below should be run from [Kata-BabyStepsGolf/cpp]() directory
...
`Kata-BabyStepsGolf` / `cpp` / `build` / `Kata-BabyStepsGolf.sln`

After loading the solution:

1. Make sure the 'Kata-BabyStepsGolf-test' project is set as the Start-up Project.
...
`File` > `Open` > `Kata-BabyStepsGolf` > `cpp`

CLion should automatically build the CMake file.

Run the test file [BabyStepsGolfTest.cpp](./test/BabyStepsGolfTest.cpp)
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `cpp`
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-BabyStepsGolf` / `cpp` / `build` / `Kata-BabyStepsGolf.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-BabyStepsGolf-test` to set it as the current active scheme.
