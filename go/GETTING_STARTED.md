# Getting Started with BabyStepsGolf kata in Go
...
> ***Reminder***: the commands below should be run from [Kata-BabyStepsGolf/go](.) directory
...
> ***Reminder***: the command below should be run from [Kata-BabyStepsGolf/go](.) directory
...
`File` > `Open` > `Kata-BabyStepsGolf` > `go`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `go`
