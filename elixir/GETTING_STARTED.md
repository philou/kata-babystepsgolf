# Getting Started with BabyStepsGolf kata in Elixir
...
> ***Reminder***: the commands below should be run from [Kata-BabyStepsGolf/elixir](.) directory
...
> ***Reminder***: the command below should be run from [Kata-BabyStepsGolf/elixir](.) directory
...
`File` > `Open` > `Kata-BabyStepsGolf` > `elixir`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-BabyStepsGolf` / `elixir`
